...
export interface CvApplicationSource {
    id?: string | number;
    jobId?: string | number;
...
interface CvCandidate extends CvApplicationSource {
    id: string | number;
    nombre: string;
    profesion: string;
    vacante: string;
    ...
    cv: string;
    experiencia: string;
    jobData: CvJobSource | null;
}

interface CandidateHistory {
    candidato: CvCandidate;
    historial: CvCandidate[];
}

interface CvReceivedProps {
    applications?: CvApplicationSource[];
    jobs?: CvJobSource[];
}

import "../../styles/admin/cv-recibidos.css";

/** Preserved local CV review prototype. */
function CVRecibidos({
    ...
    const [perfilSeleccionado, setPerfilSeleccionado] =
        useState<CvCandidate | null>(null);

    const [postulacionesSeleccionadas, setPostulacionesSeleccionadas] =
        useState<CandidateHistory | null>(null);

    // =====================================================
    // OBTENER VACANTE RELACIONADA
    // =====================================================

    const obtenerVacante = useCallback((application: CvApplicationSource): CvJobSource | null => {
        if (!application) {
            return null;
        }
    ...
    const candidatos = useMemo<CvCandidate[]>(() => {
    ...
    const abrirPerfil = (postulante: CvCandidate) => {
